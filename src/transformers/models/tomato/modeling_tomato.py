...
def write_to_file(file_path, current_path, content, mode='w', add_timestamp=True):
...
_CONFIG_FOR_DOC = "TomatoConfig"


TOMATO_START_DOCSTRING = r"""
...
@add_start_docstrings(
    "The bare Tomato Model outputting raw hidden-states without any specific head on top.",
    TOMATO_START_DOCSTRING,
)
class TomatoPreTrainedModel(PreTrainedModel):
    config_class = TomatoConfig
    base_model_prefix = "tomato"
...
class TomatoForCausalLM(TomatoPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`TomatoDecoderLayer`]

    Args:
        config: TomatoConfig
    """

    def __init__(self, config: TomatoConfig):
    ...
    @add_start_docstrings_to_model_forward(TOMATO_INPUTS_DOCSTRING)
    def forward(
        self,
