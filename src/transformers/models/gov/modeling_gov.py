...
@auto_docstring(
    custom_intro="""
    Base class for Gov outputs, with hidden states and attentions.
    """
)
class GovModelOutputWithPast(BaseModelOutputWithPast):
...
class GovVisionRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        GovVisionRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class GovVisionAttention(nn.Module):
    """Attention Class for Gov Vision Encoder"""

    def __init__(self, config: GovVisionConfig):
        super().__init__()
        self.config = config
        ...
        self.q_norm = GovVisionRMSNorm(self.embed_dim) if qk_norm else nn.Identity()
        self.k_norm = GovVisionRMSNorm(self.embed_dim) if qk_norm else nn.Identity()
...
class GovVisionPatchEmbeddings(nn.Module):
    """
...
class GovVisionEmbeddings(nn.Module):
    """
    Construct the CLS token, position and patch embeddings. Optionally, also the mask token.

    """

    def __init__(self, config: GovVisionConfig) -> None:
        super().__init__()
        ...
        self.patch_embeddings = GovVisionPatchEmbeddings(config)
...
NORM2FN = {"layer_norm": nn.LayerNorm, "rms_norm": GovVisionRMSNorm}


class GovVisionLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: GovVisionConfig) -> None:
        ...
        self.seq_len_dim = 1
        self.attention = GovVisionAttention(config)
        self.mlp = GovVisionMLP(config)
        # Gov uses different layernorm implementations for different models
    ...
    def forward(
        ...
        attention_output, attention_weights = self.attention(
            self.layernorm_before(hidden_states),  # in GovVision, layernorm is applied before self-attention
        ...
        # in GovVision, layernorm is also applied after self-attention
...
@auto_docstring
class GovVisionPreTrainedModel(PreTrainedModel):
    config: GovVisionConfig
    base_model_prefix = "gov_vision"
    main_input_name = "pixel_values"
    supports_gradient_checkpointing = True
    _no_split_modules = ["GovVisionLayer"]
    _supports_sdpa = True
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GovVisionEmbeddings):
            module.cls_token.data.zero_()
            if module.mask_token is not None:
                module.mask_token.data.zero_()
            if module.position_embeddings is not None:
                module.position_embeddings.data.zero_()
        elif isinstance(module, GovVisionLayer):
...
@auto_docstring(
    custom_intro="""
    Class for outputs of [`GovVisionModel`].
    """
)
class GovVisionModelOutputWithPooling(BaseModelOutputWithPooling):
    r"""
...
class GovVisionEncoder(nn.Module):
    def __init__(self, config: GovVisionConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([GovVisionLayer(config) for i in range(config.num_hidden_layers)])
...
@auto_docstring
class GovVisionModel(GovVisionPreTrainedModel):
    def __init__(self, config: GovVisionConfig) -> None:
        super().__init__(config)
        self.config = config

        self.embeddings = GovVisionEmbeddings(config)
        self.encoder = GovVisionEncoder(config)
    ...
    def forward(
    ...
    ) -> Union[tuple, GovVisionModelOutputWithPooling]:
        r"""
        ...
        return GovVisionModelOutputWithPooling(
...
@auto_docstring(
    custom_intro="""
    The Gov model which consists of a vision backbone and a language model, without a language modeling head.
    """
)
class GovModel(GovPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
    ) -> Union[tuple, GovModelOutputWithPast]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
...
@auto_docstring(
    custom_intro="""
    Base class for Gov causal language model (or autoregressive) outputs.
    """
)
class GovCausalLMOutputWithPast(ModelOutput):
...
@auto_docstring(
    custom_intro="""
    The GOV model which consists of a vision backbone and a language model.
    """
)
