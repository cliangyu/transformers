...
class GovImagesKwargs(ImagesKwargs, total=False):
    crop_to_patches: Optional[bool]
    min_patches: Optional[int]
    max_patches: Optional[int]


class GovProcessorKwargs(ProcessingKwargs, total=False):
    images_kwargs: GovImagesKwargs
    _defaults = {
        "text_kwargs": {
            "padding_side": "left",
...
class GovProcessor(ProcessorMixin):
    r"""
    Constructs a GOV processor which wraps a [`AutoImageProcessor`] and
    [`PretrainedTokenizerFast`] tokenizer into a single processor that inherits both the image processor and
    tokenizer functionalities. See the [`~GovProcessor.__call__`] and [`~GovProcessor.decode`] for more information.
    Args:
        ...
        for prompt in text:
            ...
            while self.image_token in new_prompt or self.video_token in new_prompt:
                if self.image_token in new_prompt and (
                    ...
                    # As of now, GOV only supports one patch per frame, but we keep the code flexible for future updates
    ...
    def __call__(
        ...
        **kwargs: Unpack[GovProcessorKwargs],
    ) -> BatchFeature:
        """
        Main method to prepare for the model one or several sequences(s) and image(s). This method forwards the `text`
        and `kwargs` arguments to PreTrainedTokenizerFast's [`~PreTrainedTokenizerFast.__call__`] to encode the text if `text`
        is not `None`. To prepare the vision inputs, this method forwards the `images` and `kwargs` arguments to
        the image processor's [`~AutoImageProcessor.__call__`] if `images` is not `None`.
    ...
    def _get_num_multimodal_tokens(self, image_sizes=None, **kwargs):
        ...
        if image_sizes is not None:
            images_kwargs = GovProcessorKwargs._defaults.get("images_kwargs", {})
